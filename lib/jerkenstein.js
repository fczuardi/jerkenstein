  ...
  jerk(function(j) {
    for (var i = plugins.length-1; i > 0; i--){
      ...
      var extendedAction = function(env, message){
        message = replaceSayMethod(message, _current_job, this.name)
...
function replaceSayMethod(message_obj, job_id, plugin_name){
  var extendedSay = function(job_id, plug_name, text){
    if (!_running_jobs[job_id].output_locked) {
      jerkensteinSay(job_id, plug_name, text)
    }else{
      queueOutput(job_id, plug_name, text)
    }
  }
  var extendedMute = function(job_id, plug_name){
    _running_jobs[job_id].muteList[plug_name] = true
  }
  message_obj.say = extendedSay.bind(null, job_id, plugin_name)
  message_obj.mutePlugin = extendedMute.bind(null, job_id)
