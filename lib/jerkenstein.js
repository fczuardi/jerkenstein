    ...
    while (i--){
      ...
      var wrapped_action = function(m){
            _plugs[index].action({
              ...
              ,plugs:_plugs
              ,say:function(text){
                _output[m.source][_plugs[index].name].push(text);
              }
            });
          }
...
function afterLastMatch(m){
  ...
  for(var i in _output[m.source]){
    var outputs_from_command = _output[m.source][i]
    for(var j in outputs_from_command){
      m.say(i+','+j+'>>'+_output[m.source][i][j]);
    }
  }
}